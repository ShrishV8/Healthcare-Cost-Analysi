...
## 1b) Checked and Cleaned NA values
```{r}
...
#Step 3: Distribution of cost column
```{r}
#Checking the shape of the cost column distribution
quantile(health_data$cost)

#Checking the shape of the cost column distribution (with 10% distribution interval)
quantile(health_data$cost,seq(0.1,1,0.1))

#Visualizing the spread of cost column
...
#Step 3a) Creating 'expensive' column
```{r}
#Creating a new column named 'expensive', if cost > 5779.8, then the value is 1, otherwise 0
health_data$expensive <- ifelse(health_data$cost > 5779.8, 1, 0)
#Checking the distribution of the new column
table(health_data$expensive)
```

## Step 5 - Creating groups for age and bmi columns
```{r}
#Grouping age column with a gap of 10 years
health_data$age_group <- cut(health_data$age, breaks = seq(10, 70, 10))
#Checking the distrbution of new age column
table(health_data$age_group)

#Grouping bmi column with a gap of 5kg/m2
health_data$bmi_group <- cut(health_data$bmi, breaks = seq(15, 55, 5))
#Checking the distrbution of new bmi column
...
```{r}
library(Rcmdr)
numSummary(health_data[,c("age","bmi","children","cost"), drop = FALSE],
           statistics = c("mean","sd","IQR","skewness","kurtosis"), type = "2")
```


#Correlation Matrix 
```{r}
myData <- health_data[,c("age","bmi","children","cost")]
res <- cor(myData)
round(res,4)
```
...
```{r}
library(ggplot2)
BoxPlot1 <- ggplot(health_data, aes(x=expensive, y=age, fill=expensive)) + aes(group=expensive) + geom_boxplot()
BoxPlot1
```
...
hist(health_data$age, main = "Age Distribution", xlab = "Age", col = "orange")
```
...
```
## Barplot to show the distribution of age_group by expensive
```{r}
...
barplot(table(health_data2$expensive, health_data2$exercise), beside
        = TRUE, col = c("orange", "blue"),
        legend = rownames(table(health_data2$expensive,health_data2$exercise)))
```
...
table(health_data2$expensive,health_data2$education_level)
#From the visualization below we can see that the expensive column's values are equally distributed in each
#education level.
```
# 4l) Barplot for hypertension
```{r}
barplot(table(health_data2$expensive, health_data2$hypertension), beside
        = TRUE, col = c("orange", "blue"),
        legend = rownames(table(health_data2$expensive,health_data2$hypertension)))
table(health_data2$expensive, health_data2$hypertension)
```


#Map graph
```{r}
#USA map plot from which states the cost data has been collected.
library(tidyverse)
us <- map_data("state")
...
map
#We can see the data is collected from small number of states so location bias can happen if we train the model on this dataset.
...
# From the below tree, we can know that important columns are: smoker, bmi_group, age_group, exercise
```
...
#The sensitivity accuracy is 98.08%, accuracy is 91.16%. 70% train data is better than 80% train data
```
...
# The sensitivity accuracy is 97.53%, the accuracy is 89.75%
```
...
#Splitting dataset 
health_data_lr <- health_data[,-c(1,14,16,17)]
#Logistic Regression
mylogit <- glm(expensive ~ ., data = health_data_lr, family = "binomial")
summary(mylogit)

#Splitting data into testing and training data
health_data_lr$expensive <- as.factor(health_data_lr$expensive)
training1 <- createDataPartition(y=health_data_lr$expensive,p=.70,list=FALSE)
train1 <- health_data_lr[training1,] # training set
test1 <- health_data_lr[-training1,] # test set

#Creating a SVM model to predict the expensive column
library(e1071)
svm_model_lr <- train(expensive ~ ., data = train1, method = "svmRadial", preProcess = c("center", "scale"))
# predict the expensive column for the test data
svm_pred_lr <- predict(svm_model_lr, test1)
# create the sensitivity confusion matrix
confusionMatrix(svm_pred_lr,test1$expensive)

#The overall accuracy is 90.19% and sensitivity is 97.09%
```
...
#Splitting dataset 
health_data_lr1 <- health_data[,-c(1,14,16,17)]
#Logistic Regression
mylogit1 <- glm(expensive ~ age + bmi + children + smoker + yearly_physical + exercise + hypertension, data = health_data_lr1, family = "binomial")
summary(mylogit1)

#Splitting dataset into training and testing data
health_data_lr1$expensive <- as.factor(health_data_lr1$expensive)
training1 <- createDataPartition(y=health_data_lr1$expensive,p=.70,list=FALSE)
train1 <- health_data_lr1[training1,] # training set
test1 <- health_data_lr1[-training1,] # test set

# create a svm model to predict the expensive column
library(e1071)
svm_model_lr1 <- train(expensive ~ age + bmi + children + smoker + yearly_physical + exercise + hypertension, data = train1, method = "svmRadial", preProcess = c("center", "scale"))
# predict the expensive column for the test data
svm_pred_lr1 <- predict(svm_model_lr1, test1)
# create the sensitivity confusion matrix
confusionMatrix(svm_pred_lr1,test1$expensive)

#The overall accuracy is 90.11% and sensitivity is 98.02%
```
...
health_data_N <- health_data[,-c(1,14,16,17)]
health_data_N$smokerd <- ifelse(health_data_N$smoker == "yes", 1, 0)
health_data_N$yearly_physicald <- ifelse(health_data_N$yearly_physical == "Yes", 1, 0)
health_data_N$exercised <- ifelse(health_data_N$exercise == "Active", 1, 0)

healthNN <- neuralnet(expensive ~ age + bmi + children + smokerd + yearly_physicald + exercised + hypertension, health_data_N, hidden = 2, lifesign = "minimal", linear.output = FALSE, threshold = 0.01)
plot(healthNN)
```
